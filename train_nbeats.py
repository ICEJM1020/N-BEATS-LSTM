...
if __name__ == '__main__': 
    ...
    ## training
    # model = NBEATS(INPUT_STEPS, OUTPUT_STEPS, len(FEA), len(FEA), layer_width=LSTM_UNITS, num_blocks=2, num_slices=NUM_SLICES)
    model = NBEATS_LSTM(INPUT_STEPS, OUTPUT_STEPS, len(FEA), len(FEA), units=LSTM_UNITS, num_blocks=2, num_slices=NUM_SLICES)
    model.build((BATCH_SIZE, INPUT_STEPS, len(FEA)))
    model.summary()
    history = model.custom_train(data_generator=win_generator, epochs=EPOCHS, lr=LEARNING_RATE)
    print(model.evaluate(win_generator.train))
    print(model.evaluate(win_generator.val))

    ## save models and history
    pd.DataFrame(history).to_csv('output/train_nbeats/nbeats_his.csv')
    model.save_weights('models/nbeats_lstm.h5')
    ...
    plt.legend()
    plt.savefig('./output/train_nbeats/nbeats_loss.png')
    ## plot the mae graph
    plt.figure(dpi=200)
    ...
    plt.legend()
    plt.savefig('./output/train_nbeats/nbeats_mae.png')
    ## plot the mape graph
    plt.figure(dpi=200)
    ...
    plt.legend()
    plt.savefig('./output/train_nbeats/nbeats_mape.png')
    ...
    plt.savefig('./output/train_nbeats/nbeats_prediction.png')
